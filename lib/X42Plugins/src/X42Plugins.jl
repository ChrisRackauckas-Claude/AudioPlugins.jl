...
!!! warning "MIT package, GPL-2.0-or-later binaries in your process"
    ...
    trees contain GPL-3.0-or-later files. Submodules that do (darc, dpl, fat1,
    meters, sisco, zconvo) are not in this artifact; meters and sisco also cannot
    build headless. `LICENSE.md` and the README carry the evidence.
...
`X42Plugins_jll` (i.e. `…/lib/lv2`). Pass it to [`AudioPlugins.lv2_default_path`](@ref)
so lilv also sees the LV2 specification bundles from `lv2_jll`.
"""
function lv2_dir()
    # balance_lv2 is …/lib/lv2/balance.lv2/manifest.ttl
