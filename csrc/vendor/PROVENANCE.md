...
| Vendored | 2026-08-29, three headers copied verbatim from `include/lv2/`: `core/lv2.h`, `urid/urid.h`, `buf-size/buf-size.h`. 2026-09-23, four more verbatim for atom-port support: `atom/atom.h`, `atom/util.h`, `midi/midi.h`, `resize-port/resize-port.h` |
...
the host features `lv2_host.c` offers, `atom/atom.h` + `atom/util.h` for the
`atom:Sequence` buffers the host connects to `atom:AtomPort`s, `midi/midi.h`
for `midi:MidiEvent` (the event type `lv2_in_midi` writes and the test
plugins read), and `resize-port/resize-port.h` for `rsz:minimumSize` (the
port property the buffers are sized from). The full specification — every
other extension header plus the Turtle bundles lilv reads at run time —
comes from `lv2_jll`, and `lv2_host.c` is built against lilv (`Lilv_jll`),
